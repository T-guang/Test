...
- 模板 ID：motor_jog_control
- 分类：工业电路
- 难度：初级
- 元件数量：7
- 导线数量：17
- 使用的器件类型：AC_ThreePhase_Power；Breaker_3P；Button_Start_NO；Button_Stop_NC；Contactor_KM_380V；Fuse_3P；Motor_ThreePhase_380V
- 模板 JSON 路径：Assets/Resources/Blueprints/Templates/motor_jog_control_template.json

## 2. 元件实例表

| 实例 ID | 测试称呼（推导） | DefinitionName | 显示名称 | 位置 | 参数摘要 |
| --- | --- | --- | --- | --- | --- |
| breaker_3p_1 | 空气开关3P [breaker_3p_1]（根据实例 ID 推导） | Breaker_3P | 空气开关3P | -240, 240 | 无模板覆盖参数 |
| fuse_3p_1 | 熔断器3P(FU) [fuse_3p_1]（根据实例 ID 推导） | Fuse_3P | 熔断器3P(FU) | 120, 240 | 无模板覆盖参数 |
| km_1 | 交流接触器(KM) (380V) [km_1]（根据实例 ID 推导） | Contactor_KM_380V | 交流接触器(KM)<br>(380V) | 504, 0 | 无模板覆盖参数 |
| motor_1 | 三相异步电动机 (380V) [motor_1]（根据实例 ID 推导） | Motor_ThreePhase_380V | 三相异步电动机<br>(380V) | 360, -216 | 无模板覆盖参数 |
| power_3p_1 | 三相交流电源 [power_3p_1]（根据实例 ID 推导） | AC_ThreePhase_Power | 三相交流电源 | -600, 456 | 无模板覆盖参数 |
| start_1 | 启动按钮(NO) [start_1]（根据实例 ID 推导） | Button_Start_NO | 启动按钮(NO) | 72, -216 | 无模板覆盖参数 |
| stop_1 | 停止按钮(NC) [stop_1]（根据实例 ID 推导） | Button_Stop_NC | 停止按钮(NC) | -288, -216 | 无模板覆盖参数 |
...
| km_1 | Contactor_KM_380V | A1、A2、L1、L2、L3、T1、T2、T3 | 13、14、21、22 |
