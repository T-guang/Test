...
- 模板 ID：breaker_lamp_template
- 分类：家庭电路
- 难度：初级
- 元件数量：4
- 导线数量：5
- 使用的器件类型：AC_220V_Power；Breaker_2P；Lamp_220V；Single_Control_Switch
- 模板 JSON 路径：Assets/Resources/Blueprints/Templates/breaker_lamp_template.json

## 2. 元件实例表

| 实例 ID | 测试称呼（推导） | DefinitionName | 显示名称 | 位置 | 参数摘要 |
| --- | --- | --- | --- | --- | --- |
| breaker_1 | 空气开关2P [breaker_1]（根据实例 ID 推导） | Breaker_2P | 空气开关2P | -240, 120 | 额定电压=220V；额定电流=16A |
| lamp_1 | 电灯泡(220V) [lamp_1]（根据实例 ID 推导） | Lamp_220V | 电灯泡(220V) | 312, 120 | 额定电压=220V；功率=40W |
| power_1 | 220V电源 [power_1]（根据实例 ID 推导） | AC_220V_Power | 220V电源 | -552, 120 | 输出电压=220V |
| switch_1 | 单开单控开关 [switch_1]（根据实例 ID 推导） | Single_Control_Switch | 单开单控开关 | 24, 120 | 开关类型=1 |
...
| W-005 | breaker_1 | P2_OUT（出2） | lamp_1 | N（N） | #1A59F2 | 空气开关2P [breaker_1]（根据实例 ID 推导）.出2 → 电灯泡(220V) [lamp_1]（根据实例 ID 推导）.N |
